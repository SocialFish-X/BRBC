...
def get_latest_model_predict_data_dir(new_epochs_ckpt_dir=None):
    ...
    if new_epochs_ckpt_dir is None:
        ...
        input_new_epochs_ckpt = os.path.join(new_epochs_ckpt_dir)
        # 获取最新周期下最新模型文件路径
        new_epochs_ckpt_dir = new_report(input_new_epochs_ckpt)
    return input_new_epochs_ckpt

#对于没有预测出关系的句子，启发式地选择相对概率最大的10个关系作为输出
def replace_empty_infer_predicate_to_three_possible_values(predicate_score_value):
    label_list = ['别名','工作频段','研发公司','所属国家','部署平台','组成单元','装备种类','服役单位','研发时间','参加战役',
...
if __name__=="__main__":
    predicate_classifiction_input_file_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "../predicate_classifiction/XMW_classification_data/test"))
